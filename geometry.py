...
class Rect():
	...
	def aligh_full_rect(self, rect):
		rectl = rect.l_top.x
		rectt = rect.l_top.y
		rectr = rect.l_top.x + rect.width
		rectb = rect.l_top.y + rect.height
	
		
		selfl = self.l_top.x
		selft = self.l_top.y
		selfr = self.l_top.x + self.width
		selfb = self.l_top.y + self.height
		
		outleft   = min(rectl, selfl)
		outright  = max(rectr, selfr)
		outbottom = min(rectb, selfb)
		outtop    = max(rectt, selft)
		
		s = Rect(outleft,outtop,(outright - outleft),(outbottom - outtop))
		#if(self.l_top.x < rect.l_top.x):
			#self.l_top.x = self.l_bot.x = self.l_top.x
		#else:
			#self.l_top.x = self.l_bot.x = rect.l_top.x
		#if(self.l_top.y < rect.l_top.y):
			#self.l_top.y = self.l_top.y #self.l_top.y = self.l_bot.y = self.l_top.y
		#else:
			#self.l_top.y = rect.l_top.y
		
		
		#self.l_top.y = self.r_top.y = rect.r_top.y
		#self.l_bot.y = self.r_bot.y = self.l_top.y + self.l_bot.y + rect.l_bot.y
		return s
